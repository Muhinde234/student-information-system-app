...
const userController ={
  ...
  try {
    // Send OTP email
    await sendEmail(emaill, 'Echo:Verify your account', emailBody);
...
updateUser: asyncWrapper(async (req, res, next) => {
    ...
    if (req.file) {
        try {
            const images = `IMAGE_${Date.now()}`;
            const ImageCloudinary = await cloudinary.v2.uploader.upload(req.file.path, {
                folder: 'Echo', // Your specified Cloudinary folder
                public_id: images
            });
    ...
    ForgotPassword : asyncWrapper(async (req, res, next) => {
      ...
      await sendEmail(req.body.email, "Echo-Reset password", emailBody);
